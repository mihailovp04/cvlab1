...
- *apache2* — устанавливает веб-сервер Apache. Это популярное программное обеспечение для хостинга веб-сайтов и обработки HTTP-запросов.
- *php* — устанавливает интерпретатор PHP, который используется для обработки серверных скриптов на стороне сервера.
- *libapache2-mod-php* — это модуль, который позволяет интегрировать PHP с веб-сервером Apache. Он позволяет обрабатывать PHP-скрипты при запросах к серверу.
- *php-mysql* — устанавливает расширение для PHP, которое позволяет взаимодействовать с базами данных MySQL или MariaDB.
- *mariadb-server* — устанавливает сервер базы данных MariaDB, который является форком MySQL. Он используется для хранения и управления данными веб-приложений.
- *mariadb-client* — клиентская часть для работы с MariaDB, которая позволяет подключаться к серверу баз данных MariaDB для выполнения запросов.
- *unzip* — утилита для разархивации файлов в формате ZIP.
